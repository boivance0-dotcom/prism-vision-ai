...
const ResultListItem: React.FC<{
  ...
  accentColor?: string;
  isLocked?: boolean;
}> = ({ item, onView, accentColor = '#86C232', isLocked = false }) => {
  const badgeClass = TYPE_BADGE[item.type] || TYPE_BADGE.project;
  const confidencePct = Math.round((item.confidence ?? 0.9) * 100);

  return (
    <article className="group relative rounded-xl p-4 bg-[rgba(7,16,12,0.65)] border border-white/15 shadow-[0_14px_40px_rgba(0,0,0,0.4)] backdrop-blur-md hover:shadow-[0_18px_56px_rgba(0,0,0,0.55)] transition-colors">
      <div className="flex items-start gap-4">
        <div className="relative shrink-0 w-16 h-16 rounded-lg overflow-hidden border border-white/15 bg-white/5">
          <div className="absolute inset-0 grid place-items-center text-white/60 text-xs uppercase tracking-wide">
            {item.type}
          </div>
        </div>
